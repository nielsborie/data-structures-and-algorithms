...
class BinarySearchRecursiveVersion(AbstractBinarySearch):
    ...
    def binary_search_helper(self, arr: list[int], target: int, left_pointer: int, right_pointer) -> int:
        ...
        if arr[middle_pointer] == target:
            ...
            return self.binary_search_helper(arr=arr, target=target, left_pointer=left_pointer,
                                             right_pointer=middle_pointer - 1)
        else:
            return self.binary_search_helper(arr=arr, target=target, left_pointer=middle_pointer + 1,
